...
=======
  ...
  Player get player => world.player; // Get reference to the player

  /// Method to trigger player's jump
  void jump() {
    player.jump(Vector2(0, -1)); // Jump straight up
  }

  /// Makes the player quickly drop down
  void drop() {
    player.drop(Vector2(0, 1)); // Move downward
  }

  /// Makes the player dash left
  void dashLeft() {
    player.dash(Vector2(-12, 1)); // Quick left move
  }

  /// Makes the player dash right
  void dashRight() {
    player.dash(Vector2(12, 1)); // Quick right move
  }
...
=======
    final scoreText = 'Halo-Halo: 0 / ${level.winScore}';
>>>>>>> 935c058 (upload)
